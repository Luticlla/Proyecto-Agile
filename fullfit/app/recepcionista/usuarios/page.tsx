...
export default function UsuariosPage() {
  ...
  useEffect(() => {
    cargarUsuarios({ filtro_rol: filtroRol, limit: limite })
  }, [cargarUsuarios, filtroRol, limite])
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    debounceRef.current = setTimeout(() => {
      cargarUsuarios({ busqueda: value, filtro_rol: filtroRol, page: 1, limit: limite })
    }, DEBOUNCE_MS)
  }

  const handlePageChange = (newPage: number) => {
    cargarUsuarios({ busqueda, filtro_rol: filtroRol, page: newPage, limit: limite })
  }
  ...
  const handleToggleEstado = async (id: string, activo: boolean) => {
    try {
      ...
      cargarUsuarios({ busqueda, filtro_rol: filtroRol, page: result.page, limit: limite })
    } catch (error: any) {
      alert(error.message)
    }
  }
  ...
  const handleSuccessCrear = () => {
    ...
    cargarUsuarios({ busqueda, filtro_rol: filtroRol, page: 1, limit: limite })
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Personal del Sistema</h1>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="text-sm text-zinc-500 min-w-[120px] text-right">
          {textoResultados}
        </div>
      </div>
