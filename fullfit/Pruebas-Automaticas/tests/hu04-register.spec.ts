...
test.describe('HU-004: Registro de Datos Personales', () => {
  ...
  test('CP-09: Password con menos de 8 caracteres muestra error', async ({ page }) => {
    // El error se muestra en vivo al escribir (passwordError vive en el render),
    // no requiere enviar el formulario — y el submit está disabled hasta
    // validar RENIEC, así que no debemos depender de hacer click en él.
    await page.locator('#password').fill('123')
    const error = page.getByText(/al menos 8 caracteres/i)
    await expect(error.first()).toBeVisible()
  })
 
  test('CP-10: Contrasenas no coinciden muestra error', async ({ page }) => {
    // Igual que CP-09: el aviso de "no coinciden" se renderiza en vivo
    // en cuanto ambos campos tienen valor y no coinciden.
    await page.locator('#password').fill('test1234')
    await page.locator('#confirmPassword').fill('otraClave1')
  ...
  test('CP-28: DNI con caracteres no numericos bloqueado', async ({ page }) => {
    // El componente filtra letras vía JS en vez de bloquear con pattern al
    // tipear: tras filtrar "12AB3456" quedan 6 dígitos, que no calzan con
    // el patrón de 8 dígitos exactos → validity inválida igualmente.
    const dni = page.locator('#dni')
  ...
  test('CP-32: Boton submit deshabilitado con formulario vacio', async ({ page }) => {
    // No existe lógica de "foco en primer campo con error al enviar": el
    // botón está disabled mientras no se cumplan todas las condiciones
    // (incluida la validación RENIEC), así que con el formulario vacío
    // ni siquiera es posible hacer click en él. Verificamos ese estado real.
    const button = page.locator('button[type="submit"]')
