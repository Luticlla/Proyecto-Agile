...
test.describe('HU-002: Listado de Sedes', () => {
  ...
  test('HU02-CP-14: Colapso panel expandido', async ({ page }) => {
    const expandBtn = page.locator('button').filter({ hasText: /ver más información|ver mas informacion/i }).first()
    await expect(expandBtn).toBeVisible({ timeout: 5000 })
    await expandBtn.click({ force: true })
    await page.waitForTimeout(500)
  ...
  test('HU02-CP-17: Mensaje sin sedes disponibles cuando BD vacia', async ({ page }) => {
  ...
  test('HU02-CP-21: Diseno responsivo en movil', async ({ page }) => {
  ...
  test('HU02-CP-22: Pagina carga con cliente anonimo (sin sesion)', async ({ page }) => {
    await expect(page).toHaveURL('/sedes')
    await expect(page.getByRole('heading', { name: /nuestras sedes/i })).toBeVisible()
  })

  test('HU02-CP-26: Accesibilidad teclado del collapsible', async ({ page }) => {
  ...
  test('HU02-CP-27: Logo desde /sedes redirige a /', async ({ page }) => {
